...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        Action.addTarget(self, action: #selector(onActionClicked), for: .touchUpInside)
    ...
    func searchForText() {
        ...
        let option : SearchOptions = SearchOptions(searchString: "途牛", matchCase: true, wholeWords: false)
        if let regex = NSRegularExpression(options: option) {

            

            otherUses(beforeText: beforeText, range: range, regex: regex)
    ...
    func otherUses(beforeText:String, range:NSRange, regex:NSRegularExpression) {
    ...
    //替换
    func replaceString(beforeText:String, range:NSRange, regex:NSRegularExpression)
    {
        //匹配整个range
    ...
    func searchTextWithProgressOption(beforeText:String, range:NSRange, regex:NSRegularExpression)
    {
        var count = 0
...
//                stop.pointee = true
    ...
    //找到后才会调用
    func searchTextWithCompletionOption(beforeText:String, range:NSRange, regex:NSRegularExpression) {
        var count = 110
...
extension NSRegularExpression {
    convenience init?(options: SearchOptions) {
        ...
        let regexOption : NSRegularExpression.Options = (isCaseSensitive) ? .allowCommentsAndWhitespace : .caseInsensitive
